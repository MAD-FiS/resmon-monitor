...
def get_hosts(q=None):  # noqa: E501
    ...
    for host in hosts:
        ...
        metadatas = [
            Metadata.from_dict(metadata) for metadata in api.getMetadataByHost(host)
        ]
        response.append(
            (Host(hostname=host, metrics=metrics, metadata=metadatas)).to_dict()
        )
...
def post_metric(hostname, payload):  # noqa: E501
    ...
    api.insertMeasDefinition(
        hostname, metric_id, parent_id, moving_window, interval, payload["description"]
    )

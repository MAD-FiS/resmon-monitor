...
class dbApi:
    ...
    def updateMetricInMetadata(self, hostname, metric_id, parent_id, description):
        record = self.findInMeta({self.HOSTNAME_KEY:hostname, self.METRIC_PATH:parent_id})
        ...
        self.db.update({self.HOSTNAME_KEY:hostname, self.METRIC_PATH:parent_id},
                {self.METRICS_KEY:metrics}, self.META_COLL)

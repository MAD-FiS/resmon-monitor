...
def timePoints(startTime, endTime, interval):
    ...
    return (
        startTime + timedelta(seconds=(interval * x))
    ...
    while True:
        ...
        for measDef in measDefs:
            ...
            for sessionId in sessionIds:
                ...
                cpxValues = cpxCalculator.calculate(
                    simpleMeasurements, nowTime
                )
                print(sessionId, measDef.metric_id, cpxValues)
                if len(cpxValues) > 0:
                    api.insertMeasurements(
                        sessionId, measDef.metric_id, cpxValues
                    )
            measDef.lastCalcTime = nowTime
            api.updateMeasDefinition(
                measDef.hostname,
                measDef.metric_id,
                measDef.parent_id,
                measDef.movingWindow,
                measDef.interval,
                measDef.description,
                measDef.lastCalcTime,
                measDef.owner

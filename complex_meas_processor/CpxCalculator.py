...
class CpxCalculator:
    ...
    def calculate(self, simpleMeasurements, nowTime):
        return self.getCpxValues(self.measDef.lastCalcTime, nowTime, self.measDef.interval, self.measDef.movingWindow, simpleMeasurements)
    ...
    def getCpxValues(self, startTime, endTime, interval, movingWindow, measurements):
        ...
        for timePoint in timePoints(startTime, endTime, interval):
            ...
            simpleValues = self.getSimpleValues(windowStartTime, windowEndTime, measurements)
            complexValue = self.calcCpx(simpleValues)
            cpxValues.append([complexValue, timePoint + timedelta(seconds=movingWindow)])
        return cpxValues

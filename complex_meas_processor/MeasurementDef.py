...
class MeasurementDef:
    ...
    def getFilter(self):
        measFilter = dict()
        measFilter[dbApi.dbApi.NAME_KEY] = self.hostname
        measFilter[dbApi.dbApi.METRIC_PATH] = self.parent_id
        return measFilter

    @classmethod
    def fromDict(cls, cpx):
        hostname = cpx[dbApi.dbApi.NAME_KEY]
        ...
        return cls(hostname, metric_id, parent_id, movingWindow, interval, lastCalcTime, description)
